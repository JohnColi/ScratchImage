...
public class ScratchImage : MonoBehaviour
{
    ...
    /// <summary> 直方图桶的数量，必须与shader中定义的一致, 且小于256 </summary>
    public const int HISTOGRAM_BINS = 128;

    /// <summary> 用来控制透明度的RT相比Image尺寸的比例，值越小性能越高，但是精度和效果也越差 </summary>
    public const float ALPHA_RT_SCALE = 0.4f;

    /// <summary> 每一批次的实例数量上限（太多有些设备会有异常） </summary>
    ...
    public StatData GetStatData()
    {
        ...
        ret.fillPercent = 1.0f - _histogramData[0] / (dispatchCount * 1.0f); // 準0硉掀瞰
    ...
    private bool BuildCommands()
    {
        ...
        float len = fromToVec.magnitude;    //兩點的距離
    ...
    private void Init()
    {
        ...
        //Clone Mat 
        Material maskMat = Instantiate(maskImage.material);
    ...
    private void CheckInput()
    {
        ...
        EMouseState mouseStatus = EMouseState.none;// 0:none, 1:down, 2:hold, 3:up

        if (Input.GetMouseButtonDown(0)) // 按下鼠标
            mouseStatus = EMouseState.down;
        else if (Input.GetMouseButton(0)) // 移动鼠标或者处于按下状态
            mouseStatus = EMouseState.hold;
        else if (Input.GetMouseButtonUp(0)) // 释放鼠标
            mouseStatus = EMouseState.up;

        if (mouseStatus == EMouseState.none)
            return;
